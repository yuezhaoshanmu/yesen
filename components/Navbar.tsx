...
export default function Navbar(){
 ...
 useEffect(()=>{
   let frame=0;
   const scroll=()=>{if(frame)return;frame=requestAnimationFrame(()=>{setScrolled(window.scrollY>30);const total=document.documentElement.scrollHeight-window.innerHeight;if(progress.current)progress.current.style.transform=`scaleX(${total>0?window.scrollY/total:0})`;frame=0;});};
